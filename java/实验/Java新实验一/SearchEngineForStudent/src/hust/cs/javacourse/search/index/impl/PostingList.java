...
public class PostingList extends AbstractPostingList  {
    ...
    public void add(List<AbstractPosting> postings) {
       /* List temp=new ArrayList(this.list);
        temp.retainAll(postings);
        this.list.removeAll(temp);
        this.list.addAll(postings);*/
        Iterator<AbstractPosting> iterator=postings.iterator();
    ...
    public int indexOf(int docId) {
        int i=0;
        while (this.list.get(i).getDocId()!=docId&&i<this.list.size())
            i++;
        if(i==this.list.size())
            return -1;
        else return i;
    ...
    @Override
    public void sort() {
        this.list.sort(AbstractPosting::compareTo);
    }
