...
char* cpu_usage()//cpu使用率
{
	...
	else
	{
		...
		idle_c=idle_t-idle;	
		cpu_used_percent=(float)100*(total_c-idle_c)/total_c;
		total=total_t;	//此次数据保存
		idle=idle_t;
	}
	close(fp);		
	sprintf(temp_cpu,"cpu使用率：%0.2f%%",cpu_used_percent);
	return temp_cpu;
}	
...
{
	gtk_label_set_text(GTK_LABEL(cpu_label),cpu_usage());
...
char *get_cpu_name(char *_buf1)//获取CPU名称
{
	FILE * fp;
	int i=0;
	char *buf1=_buf1;
...
char *get_cpu_type(char *_buf2)//获取CPU类型
{
	FILE * fp;
	int i=0;
	char *buf2=_buf2;
...
char *get_system_type(char *_buf1)//获取操作系统类型
{
	FILE * fp;
   int i=0;
	char *buf1=_buf1;
...
char *get_system_version(char *_buf2)//获取操作系统版本
{
	FILE * fp;
   int i=0;
   int j=0;
...
void get_proc_info(GtkWidget *clist,int *p,int *q,int *r,int *s)//获取进程信息
{
   DIR *dir;
   struct dirent *ptr;
   int i,j;
...
void get_module_info(GtkWidget *clist2)//获取模块信息
{
	FILE *fp;
   int i;
   char _buffer[1024];
...
{
	gtk_clist_get_text(GTK_CLIST(clist),row,column,&txt_pid1);
	printf("%s\n",txt_pid1);
}
...
int main( int argc,char *argv[] )
{
    ...
    sprintf(bufferf1, "    CPU名称：%s\n    CPU类型：%s\n",get_cpu_name(buf1),get_cpu_type(buf2));
    ...
    sprintf(bufferf1, "    操作系统类型：%s\n    操作系统版本：%s\n",get_system_type(buf1),get_system_version(buf2));
    label = gtk_label_new (bufferf1);
    ...
    sprintf(bufferf3, "Monitor0.1 Version\t\t\t\n\nCopyright@2020  yifei\t\t\t");
