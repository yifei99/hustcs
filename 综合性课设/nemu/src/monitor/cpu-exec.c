...
void cpu_exec(uint64_t n)
{
  ...
  for (; n > 0; n--)
  {
    ...
    {
      bool success = true;
      uint32_t val_now = expr(p->next->expression, &success);
      if (success)
      {
        if (val_now != p->next->old_val)
        {
          change = true;
          printf("Watchpoint %d %s changed: old %08x -> new %08x\n", p->next->NO, p->next->expression, p->next->old_val, val_now);
          p->next->old_val=val_now;
        }
        p=p->next;
      }
      else
      {
        printf("Watchpoint %d 's expression failed :%s\n", p->next->NO, p->next->expression);
